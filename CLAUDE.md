...
- **Style:** `.glassEffect(.regular.tint(.green), in: Capsule())` with `.subheadline.weight(.medium)` text; `.padding(.horizontal, 16)`, `.padding(.vertical, 10)`, `.padding(.top, 8)`. Do **not** set an explicit foreground color — Liquid Glass renders vibrant text that adapts to what is behind the banner; a hardcoded `.white` can fail contrast once the tint adapts.
...
- Liquid Glass is adopted where the app draws its own chrome. `RootView` builds its four tabs with the value-based `Tab` initializer (not `.tabItem`) and applies `.tabBarMinimizeBehavior(.onScrollDown)` so the long History and Stats lists get the full screen height. The three confirmation banners use `.glassEffect(.regular.tint(.green), in: Capsule())`; the Stats chart share buttons use `.buttonStyle(.glass)`. Everything else is standard SwiftUI and picks up glass automatically from the iOS 26 SDK, so there are no `#available` gates anywhere — the deployment target is already iOS 26. Use the Regular variant throughout; Clear is only for glass over photo or video content, which this app has none of. Never nest one glass surface inside another.
